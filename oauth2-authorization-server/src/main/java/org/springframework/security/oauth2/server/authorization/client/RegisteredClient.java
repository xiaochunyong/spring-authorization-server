...
public class RegisteredClient implements Serializable {
	...
	// 客户端的密钥。该值应该使用 Spring Security 的 PasswordEncoder 进行编码。
	private String clientSecret;
	// 客户端密码过期的时间。
	private Instant clientSecretExpiresAt;
	// 用于客户端的描述性名称。该名称可能会在某些情况下使用，例如在同意页面中显示客户端名称时。
	private String clientName;
	// 客户端可以使用的身份验证方法。支持的值为 client_secret_basic、client_secret_post、private_key_jwt、client_secret_jwt 和 none（公共客户端）。
	private Set<ClientAuthenticationMethod> clientAuthenticationMethods;
	// 客户端可以使用的授权授予类型。支持的值为 authorization_code、client_credentials 和 refresh_token。
	private Set<AuthorizationGrantType> authorizationGrantTypes;
	// 客户端可以在基于重定向的流中使用的已注册重定向 URI——例如，authorization_code grant。
	private Set<String> redirectUris;
	// 允许客户端请求的范围。
	private Set<String> scopes;
	// 客户端的自定义设置——例如，需要 PKCE、需要授权同意等。
